...
bulk insert TMP_Naprawa from 'C:\Users\t-ja1\OneDrive\Pulpit\etl\repairs_data_t2'
WITH (
	FIRSTROW = 2,
	FIELDTERMINATOR = ',',
	ROWTERMINATOR = '\n',
...
SELECT
	[Koszt] = TMP.Koszt,
	[ID_Samochod] = (SELECT Samochod.Samochod_ID FROM Samochod WHERE Samochod.Nr_VIN = TMP.Samochod AND Samochod.Czy_aktywny = 1),
...
GO
-- select * from Samochod where Samochod.Nr_vin = '3N1BB51A2Y16T5239' order by Nr_vin
-- select * from Samochod where Samochod.Czy_aktywny = 0
-- select Nr_vin, COUNT(Nr_VIN) from Samochod WHERE Samochod.Czy_aktywny = 1 GROUP BY Nr_vin
