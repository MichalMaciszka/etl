...
bulk insert dbo.cars_excel from 'C:\Users\t-ja1\OneDrive\Pulpit\etl\cars_EXCEL_data_t2'
WITH (
	FIRSTROW = 2,
	FIELDTERMINATOR = ',',
	ROWTERMINATOR = '\n',
...
MERGE INTO Samochod as SD USING tmpCars_view as TMP ON SD.Nr_vin = TMP.Nr_vin
	...
	WHEN MATCHED AND ((SD.Stan_licznika <> TMP.[Stan licznika]) OR (SD.ID_termin_przegladu <> (SELECT Termin.Termin_ID FROM Termin WHERE CAST(Day(TMP.[data_ostatniego_przegladu]) AS int) = Termin.Dzien AND CAST(Datename(month, TMP.[data_ostatniego_przegladu]) as varchar(11)) = Termin.Miesiac AND CAST(Year(TMP.[data_ostatniego_przegladu]) AS int) = Termin.Rok)) OR (SD.Czy_powypadkowy <> TMP.[Czy powypadkowy]))
	THEN
