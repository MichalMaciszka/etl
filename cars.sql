...
MERGE INTO Samochod as SD USING tmp_view as TMP ON SD.Nr_vin = TMP.Nr_vin
	WHEN NOT MATCHED
	THEN
		INSERT VALUES (
			TMP.[Nr_vin],
			...
			(SELECT Termin.Termin_ID FROM Termin WHERE CAST(Day(TMP.[data_ostatniego_przegladu]) AS int) = Termin.Dzien AND CAST(Datename(month, TMP.[data_zakupu]) as varchar(11)) = Termin.Miesiac AND CAST(Year(TMP.[data_ostatniego_przegladu]) AS int) = Termin.Rok),
			1
		)
...
SELECT
	...
	(SELECT Termin.Termin_ID FROM Termin WHERE CAST(Day(TMP.[data_ostatniego_przegladu]) AS int) = Termin.Dzien AND CAST(Datename(month, TMP.[data_zakupu]) as varchar(11)) = Termin.Miesiac AND CAST(Year(TMP.[data_ostatniego_przegladu]) AS int) = Termin.Rok),
	1
FROM tmp_view as TMP, Samochod as SD
EXCEPT
SELECT
